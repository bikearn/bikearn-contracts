...
describe("Public Vesting", () => {
  before("set up", async () => {
    ...
    this.PrivateVestingV2 = await getContract(ethers, "PublicVestingV2");
  });
  ...
  it("deploy", async () => {
    ...
    const vestingDeploy = await this.PrivateVesting.contract.deploy(
      rteDeploy.address,
      busdDeploy.address,
    ...
    // const vestingV2Deploy = await this.PrivateVestingV2
    //     .contract
    //     .deploy(
    //         rteDeploy.address,
    //         vestingDeploy.address,
    //         currentTime - DAY * 80, // start buy time
    //         currentTime + DAY * 0, // claim buy time
    //         currentTime + DAY * 0 // cliff buy time
    //     )
    // await vestingV2Deploy.deployed()
  ...
  it("transfer", async () => {
    const transfered = await this.rte.transfer(
  ...
  it("approval", async () => {
    const allowance = await this.rte.allowance(
      this.signers[0].address,
      this.vesting.address
    );
    if (allowance.eq(ethers.BigNumber.from("0"))) {
      const rteApproval = await this.rte.approve(
        this.vesting.address,
        UNLIMITED_ALLOWANCE
      );
